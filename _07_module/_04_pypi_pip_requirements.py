...
# - Pythin 패키지를 올리고 내려받는 공식 저장소

# pip : PyPI에서 패키지를 검색, 설치, 삭제하는 도구

# requirements.txt : 프로젝트에 필요한 패키지 목록을 적어두는 파일
# -> 해당 패키지 목록을 이용해서 일관 설치 가능
# = 의존성 명세 파일

# 터미널에서 python -m pip install --upgrade pip 실행
...
#순서대로 터미널에 실행
pip_commands = [
    "python -m venv .venv",        # venv 라는 이름이 가상환경 생성(가상환경 폴더명 .venv)
    ...
    "python -m pip uninstall requests", #패키지 지우기
    ]

# pip list: 현재 가상환경에 설치된 패키지 리스트
...
def find_missing_packages() -> list[str]:
    ...
    """
    missing_packages = []   #설치 안된 패키지를 저장할 리스트
...
def print_import_results() -> None:
    """
    설치된 패키지를 실제 Python 모듈로 import 할 수 있는지 확인
    """
